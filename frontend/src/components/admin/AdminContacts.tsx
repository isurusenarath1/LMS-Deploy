      ...
      <div className="bg-white rounded-xl shadow-md p-4">
        <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full">
            <input value={query} onChange={e=>{setQuery(e.target.value); setPage(1);}} placeholder="Search messages" className="px-3 py-2 border rounded-lg w-full sm:w-64" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {/* Left: list */}
          <div className="col-span-1 lg:border-r lg:pr-2">
            <div className="space-y-2">
              {paged.map(m => (
                <button key={m._id} onClick={()=>setSelected(m)} className={`w-full text-left p-2 sm:p-3 rounded-lg flex justify-between items-start ${selected && selected._id === m._id ? 'bg-blue-50' : m.read ? 'bg-white' : 'bg-gray-50 hover:bg-gray-100'}`}>
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <div className="font-medium text-sm truncate">{m.subject}</div>
                      {!m.read && <span className="text-[10px] bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">New</span>}
                    </div>
                  ...
                  <div className="ml-2 text-[11px] text-gray-400">{new Date(m.createdAt).toLocaleString()}</div>
                </button>
              ))}
            ...
            {selected ? (
              <div className="p-3">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div>
                    <h2 className="text-lg sm:text-xl font-semibold">{selected.subject}</h2>
                  ...
                  <div className="flex items-center gap-2">
                    {!selected.read && <button onClick={()=>{handleMarkRead(selected._id); setSelected({...selected, read:true});}} className="px-2 py-1 text-sm bg-green-600 text-white rounded">Mark read</button>}
                    <button onClick={()=>{handleDelete(selected._id);}} className="px-2 py-1 text-sm bg-red-100 text-red-700 rounded flex items-center gap-2"><TrashIcon className="w-4 h-4" />Delete</button>
                  </div>
                </div>
                <div className="mt-3 p-3 bg-gray-50 rounded text-sm whitespace-pre-wrap">{selected.message}</div>
              </div>
            ) : (
              <div className="p-4 text-gray-600">Select a message to view details</div>
            )}
          </div>
        </div>
      </div>
