...
export default function AdminPayments() {
  ...
  return <div>
      ...
      opacity: 1,
      y: 0
    }} className="bg-white p-4 md:p-6 rounded-xl shadow-md mb-8">
      ...
      <div className="bg-white rounded-xl shadow-md p-4 md:p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input type="text" placeholder="Search payments..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm" />
          </div>
        </div>
        ...
        {/* Mobile: Recent Payments compact list */}
              ...
              <div key={order._id || idx} className="p-3">
                <div className="flex items-start justify-between">
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-gray-900">{order.customer ? order.customer.name : (order.user || '—')}</div>
                    <div className="text-[11px] text-gray-600 truncate mt-1">LKR {order.total} • {order.paymentMethod}</div>
                    <div className="text-[11px] text-gray-500 mt-1">{new Date(order.createdAt).toLocaleString()}</div>
                  </div>
      ...
      <div className="bg-white rounded-xl shadow-md p-4 md:p-6 mt-6">
        ...
        {/* Mobile: Cash Payments compact list */}
        <div className="block md:hidden">
          <div className="divide-y divide-gray-200">
            {payments.filter(p => (p.paymentMethod && String(p.paymentMethod).toLowerCase().includes('cash'))).map((p, idx) => (
              <div key={p._id || idx} className="p-3">
                <div className="flex items-start justify-between">
                  <div className="min-w-0">
                    <div className="text-sm font-medium">{p.customer ? p.customer.name : '—'}</div>
                    <div className="text-[11px] text-gray-600">{p.customer ? p.customer.studentId || p.customer.email : ''}</div>
                    <div className="text-[11px] text-gray-900 font-semibold mt-1">LKR {p.total}</div>
                    <div className="text-[11px] text-gray-500 mt-1">{p.payment && p.payment.reference ? `Ref: ${p.payment.reference}` : '—'}</div>
                  </div>
                  <div className="ml-3 flex-shrink-0 flex flex-col items-end space-y-2">
                    ...
                    {!p.payment || !p.payment.verified ? <button onClick={() => handleConfirm(p)} className="px-3 py-1 text-sm bg-green-600 text-white rounded">Confirm</button> : <span className="px-3 py-1 text-sm bg-green-100 text-green-800 rounded">Verified</span>}
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="bg-white rounded-xl shadow-md p-4 md:p-6 mt-6">
        ...
        {/* Mobile: Online Payments compact list */}
        <div className="block md:hidden">
          <div className="divide-y divide-gray-200">
            {payments.filter(p => (p.paymentMethod && String(p.paymentMethod).toLowerCase() === 'online')).map((p, idx) => (
              <div key={p._id || idx} className="p-3">
                <div className="flex items-start justify-between">
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-gray-600">{p.customer ? p.customer.studentId || p.customer.email : ''}</div>
                    <div className="text-[11px] text-gray-900 font-semibold mt-1">LKR {p.total}</div>
                    <div className="text-[11px] text-gray-500 mt-1">{p.payment && (p.payment.transactionId || p.payment.txnId || p.payment.reference) ? (p.payment.transactionId || p.payment.txnId || p.payment.reference) : '—'}</div>
                  </div>
                  <div className="ml-3 flex-shrink-0 flex flex-col items-end space-y-2">
                    <div className="text-xs text-gray-500">{p.createdAt ? new Date(p.createdAt).toLocaleString() : ''}</div>
                    {!p.payment || !p.payment.verified ? <button onClick={() => handleConfirm(p)} className="px-3 py-1 text-sm bg-green-600 text-white rounded">Confirm</button> : <span className="px-3 py-1 text-sm bg-green-100 text-green-800 rounded">Verified</span>}
      ...
      <div className="bg-white rounded-xl shadow-md p-4 md:p-6 mt-6">
        ...
        {/* Mobile: Submitted Bank Slips compact list */}
        <div className="block md:hidden">
          <div className="divide-y divide-gray-200">
            {payments.filter(p => (p.payment && p.payment.slipUrl) || (p.paymentMethod && String(p.paymentMethod).toLowerCase().includes('bank'))).map((p, idx) => (
              <div key={p._id || idx} className="p-3">
                <div className="flex items-start justify-between">
                  ...
                  <div className="ml-3 flex-shrink-0 flex flex-col items-end space-y-2">
                    <div className="text-xs text-gray-500">{p.createdAt ? new Date(p.createdAt).toLocaleString() : ''}</div>
                    {p.payment && (p.payment.slipUrl || p.payment.slipPath) ? <a className="text-sm text-blue-600 underline" href={p.payment.slipUrl || (window.location.origin + p.payment.slipPath)} target="_blank" rel="noreferrer">View slip</a> : <span className="text-sm text-gray-400">No slip</span>}
                    {!p.payment || !p.payment.verified ? <button onClick={() => handleConfirm(p)} className="px-3 py-1 text-sm bg-green-600 text-white rounded">Confirm</button> : <span className="px-3 py-1 text-sm bg-green-100 text-green-800 rounded">Verified</span>}
                  </div>
                </div>
              </div>
